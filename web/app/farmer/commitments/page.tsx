...
export default function Commitments(){const [rows,setRows]=useState<C[]>([]);const [msg,setMsg]=useState("Chargement…");const [busyId,setBusyId]=useState<string|null>(null);
 async function load(){try{const data=await api("/commitments");setRows(data);setMsg(data.length?"":"Aucune proposition en attente.");}catch(e){setMsg(e instanceof Error?e.message:"Erreur");}}
 useEffect(()=>{load();},[]);
 async function act(id:string,action:"accept"|"decline"){if(busyId)return;setBusyId(id);try{await api(`/commitments/${id}/${action}`,{method:"POST"});setMsg(action==="accept"?"Proposition acceptée.":"Proposition refusée. AGRI-CI cherche un remplacement.");await load();}catch(e){setMsg(e instanceof Error?e.message:"Erreur");}finally{setBusyId(null);}}
 ...
 {c.status==="PENDING"&&<div className="commitActions"><button disabled={busyId!==null} onClick={()=>act(c.id,"accept")}>{busyId===c.id?"TRAITEMENT…":"ACCEPTER"}</button><button disabled={busyId!==null} className="decline" onClick={()=>act(c.id,"decline")}>{busyId===c.id?"TRAITEMENT…":"REFUSER"}</button></div>}</article>)}</section></main>}
