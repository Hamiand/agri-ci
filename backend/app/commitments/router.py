...
def owned_commitment(db,user,commitment_id):
    ...
    commitment=db.scalar(select(Commitment).where(Commitment.id==commitment_id).with_for_update())
    if not commitment:raise HTTPException(status_code=404,detail="COMMITMENT_NOT_FOUND")
    if not farmer or commitment.farmer_id!=farmer.id:raise HTTPException(status_code=403,detail="COMMITMENT_NOT_OWNED")
    if commitment.status!="PENDING":raise HTTPException(status_code=409,detail="COMMITMENT_ALREADY_RESOLVED")
    if commitment.expires_at<datetime.now(timezone.utc):raise HTTPException(status_code=409,detail="COMMITMENT_EXPIRED")
    return commitment
...
def accept(commitment_id:uuid.UUID,db:Session=Depends(get_db),user:User=Depends(get_current_user),
    ...
    idem=begin_idempotent(db,user,f"/commitments/{commitment_id}/accept",idempotency_key,{"commitment_id":str(commitment_id),"action":"accept"})
    ...
    if offer.quantity_proposed_kg<c.quantity_kg:raise HTTPException(status_code=409,detail="QUANTITY_CONFLICT")
    offer.quantity_proposed_kg-=c.quantity_kg;offer.quantity_reserved_kg+=c.quantity_kg;offer.version+=1
    c.status="ACCEPTED";c.responded_at=datetime.now(timezone.utc)
    member.status="ACCEPTED";member.accepted_quantity_kg=c.quantity_kg
    agg.accepted_quantity_kg+=c.quantity_kg
    if agg.accepted_quantity_kg>=agg.target_quantity_kg:agg.status="CONFIRMED"
    else:agg.status="PARTIALLY_CONFIRMED"
...
def decline(commitment_id:uuid.UUID,db:Session=Depends(get_db),user:User=Depends(get_current_user),
            idempotency_key:str|None=Header(default=None,alias="Idempotency-Key")):
    idem=begin_idempotent(db,user,f"/commitments/{commitment_id}/decline",idempotency_key,{"commitment_id":str(commitment_id),"action":"decline"})
    if idem.response_body is not None:return idem.response_body
    c=owned_commitment(db,user,commitment_id)
    offer=db.scalar(select(Offer).where(Offer.id==c.offer_id).with_for_update())
    member=db.get(AggregationMember,c.aggregation_member_id)
    agg=db.scalar(select(Aggregation).where(Aggregation.id==c.aggregation_id).with_for_update())
    if offer.quantity_proposed_kg<c.quantity_kg:raise HTTPException(status_code=409,detail="QUANTITY_CONFLICT")
    offer.quantity_proposed_kg-=c.quantity_kg;offer.quantity_available_kg+=c.quantity_kg;offer.version+=1
    c.status="DECLINED";c.responded_at=datetime.now(timezone.utc);member.status="DECLINED"
    ...
    db.add(DomainEvent(event_type="COMMITMENT_DECLINED",aggregate_type="AGGREGATION",
        aggregate_id=str(agg.id),payload={"commitment_id":str(c.id),"returned_kg":float(c.quantity_kg),
                                          "replacement_proposed_kg":float(added)}))
    body={"commitment_id":str(c.id),"status":"DECLINED","returned_to_available_kg":float(c.quantity_kg),
...
def my_commitments(db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    ...
    rows=db.execute(select(Commitment,Aggregation,Demand,Offer,Product)
