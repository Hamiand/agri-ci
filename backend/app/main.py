...
app=FastAPI(title="AGRI-CI Core API",version="0.9.0",
    description="AGRI-CI: future harvest and agricultural offer foundation.")
app.add_exception_handler(HTTPException, http_exception_handler)

app.add_middleware(RequestIdMiddleware)
for router in [auth_router,farmers_router,farms_router,plots_router,harvests_router,products_router,offers_router,buyers_router,demands_router,matching_router,aggregation_router,commitments_router,orders_router,collection_router,logistics_router,deliveries_router,payments_router]:
    app.include_router(router,prefix=settings.api_v1_prefix)

@app.exception_handler(Exception)
async def unhandled_exception(request:Request,exc:Exception):
    return JSONResponse(status_code=500,content={"error":{"code":"INTERNAL_ERROR",
        "message":"Une erreur interne est survenue.","details":{},
        "trace_id":getattr(request.state,"request_id",None)}})

@app.get("/health",tags=["System"])
def health(): return {"status":"ok","application":settings.app_name,"environment":settings.app_env,"version":"0.9.0"}
